...
=============================================================
 ...
 v7 : liquid reference (LJ ε=1.5kBT) instead of WCA,
      matplotlib default style (white bg, primary colors),
      individual PNG saved per plot in ./plots/
=============================================================
 Changes from v6:
   - "no_gel" mode → weakly attractive LJ ε=1.5kBT (liquid, not WCA)
     → physically meaningful comparison: gel vs liquid, same potential form
   - All plots: white background, matplotlib default color cycle
   - Each plot saved individually to ./plots/*.png
   - All labels/titles in English
=============================================================
"""
...
EPS_GEL    = 5.0      # gel: deep attractive well → kinetic arrest
EPS_LIQ    = 1.5      # liquid reference: weak attraction, stays ergodic

DT         = 5e-5
N_EQUIL    = 8_000
N_STEPS    = 4_000_000   # t_final = 200 τ
SNAP_EVERY = 400_000     # 11 snapshots (t=0..200)
...
def run_simulation(mode="gel", seed=SEED):
    ...
    return snapshots, pos, L, pos_snaps
...
# ── 7. Summary dashboard (all panels, white) ──────────────
def plot_dashboard(snaps_gel, snaps_liq, pos_gel, pos_liq, L,
    ...
    fig.suptitle(
        f"Brownian Dynamics — Coarsening & Arrest in Colloidal Gels  [v7]\n"
        f"N={N}   φ={PHI}   dt={DT}   ε_gel={EPS_GEL}kBT   ε_liq={EPS_LIQ}kBT"
        f"   t_final={N_STEPS*DT:.0f}τ   (Euler-Maruyama + numba)",
        fontsize=10
    )
...
if __name__ == "__main__":
    ...
    snaps_gel, pos_gel, L, ps_gel = run_simulation(mode="gel")
    snaps_liq, pos_liq, _,  ps_liq = run_simulation(mode="liquid")
    ...
    plot_dashboard(snaps_gel, snaps_liq, pos_gel, pos_liq, L, ps_gel, ps_liq)
